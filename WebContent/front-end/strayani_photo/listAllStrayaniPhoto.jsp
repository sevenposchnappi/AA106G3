...
<%
    ...
    List<StrayaniPhotoVO> list = strayaniPhotoSvc.getAll();
    pageContext.setAttribute("list",list);		//如果沒有setAttribute，JLTS的for each就沒辦法跑。
%>

<html>
<head>
<title>所有送養動物照片 - listAllStrayani.jsp</title>
</head>
<body bgcolor='white'>
...
<table border='1' bordercolor='#CCCCFF' width='800'>
	...
	<c:forEach var="strayaniPhotoVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
		<tr align='center' valign='middle' ${(strayaniPhotoVO.str_Ani_Pic_No==param.str_Ani_Pic_No) ? 'bgcolor=#CCCCFF':''}>
			
			<td>${strayaniPhotoVO.str_Ani_Pic_No}</td> 
			<td>${strayaniPhotoVO.stray_Ani_Id}</td>
			<td>${strayaniPhotoVO.mem_Id}</td>
<%-- 			<td height=200px width=200px style="table-layout:fixed"><div><img onload="javascript:this.resized=true;this.style.width=300;" src="<%=request.getContextPath()%>/DBGifReader_StrayaniPhoto/DBGifReader_StrayaniPhoto.do?str_Ani_Pic_No=${strayaniPhotoVO.str_Ani_Pic_No}"></div></td> --%>
			<td  style="table-layout:fixed; height:200px ; width:300"><div><img style="table-layout:fixed; max-height:200px; max-width:200px"  src="<%=request.getContextPath()%>/front-end/DBGifReader_StrayaniPhoto/DBGifReader_StrayaniPhoto.do?str_Ani_Pic_No=${strayaniPhotoVO.str_Ani_Pic_No}"></div></td>
